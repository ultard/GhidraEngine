...
TEST_CASE("C API rejects bad arguments without crashing", "[capi]") {
    ...
    SECTION("video preview validates its arguments") {
        ...
        CHECK(ghidraengine_video_preview("missing.mp4", 320, 0.25, nullptr) == GHIDRAENGINE_ERR_INVALID_ARGUMENT);
        CHECK(ghidraengine_video_preview("missing.mp4", 0, 0.25, &preview) == GHIDRAENGINE_ERR_INVALID_ARGUMENT);
        CHECK(ghidraengine_video_preview("missing.mp4", 320, 0.25, &preview) != GHIDRAENGINE_OK);
